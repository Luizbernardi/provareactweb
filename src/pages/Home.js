...
const Home = () => {
    ...
    const totalEntradas = lista.reduce((acc, item) => acc + item.entrada, 0);
    const totalSaidas = lista.reduce((acc, item) => acc + item.saida, 0);
    const saldoAtual = totalEntradas - totalSaidas;

    return (
        <div>
            <h1 class="tituloHome">Home</h1>
            <div class="resultsHome">
                <strong>
                    <ResultsHome entradas={totalEntradas} saidas={totalSaidas} saldo={saldoAtual} /></strong>
            </div>
        </div>
    );
};
